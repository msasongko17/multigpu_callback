...
int main(int argc, char *argv[])
{
	...
	hipEvent_t *kernelEvent = (hipEvent_t *) malloc((nthreads - 1) * sizeof(hipEvent_t));
...
#pragma omp parallel num_threads(nthreads)
        {
			...
			for(int i = 0; i < nthreads - 1; i++) {
				hipEventRecord(kernelEvent[i], streams[tid]);
			}
			...
			hipSetDevice(tid);
			hipStreamWaitEvent(streams[tid], kernelEvent[tid-1],0);
